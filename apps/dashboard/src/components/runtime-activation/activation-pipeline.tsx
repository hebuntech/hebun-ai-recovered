...
import { activationPipeline } from "@/features/runtime-activation";

export function ActivationPipeline() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Activation Pipeline</CardTitle>
        <span className="text-xs text-fg-muted tabular-nums">{activationPipeline.length} steps</span>
      </CardHeader>
      <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {activationPipeline.map((step) => (
