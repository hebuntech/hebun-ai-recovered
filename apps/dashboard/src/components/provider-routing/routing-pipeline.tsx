...
import { routingPipeline } from "@/features/provider-routing";

export function RoutingPipeline() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Routing Pipeline</CardTitle>
        <span className="text-xs text-fg-muted tabular-nums">{routingPipeline.length} steps</span>
      </CardHeader>
      <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {routingPipeline.map((step) => (
