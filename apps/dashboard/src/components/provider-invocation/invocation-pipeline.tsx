...
import { invocationPipeline } from "@/features/provider-invocation";

export function InvocationPipeline() {
  return (
    <Card>
      <CardHeader>
        <CardTitle>Invocation Pipeline</CardTitle>
        <span className="text-xs text-fg-muted tabular-nums">{invocationPipeline.length} steps</span>
      </CardHeader>
      <CardContent className="grid grid-cols-1 gap-2 sm:grid-cols-2">
        {invocationPipeline.map((step) => (
